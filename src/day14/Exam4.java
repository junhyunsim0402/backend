...
public class Exam4 {
    public static void main(String[] args) {
        /*
        * Thread Pool
        *   1. 미리 일정 개수의 Thread를 생성하고 필요에 따라 재사용하는 방식
        *   2. 목적 : 작업이 들어올 때 마다 새로운 스레드가 아닌 기존 스레드를 이용
        *   3. 효과 : Thread 재사용, 자원효율성, 서버과부하 방지 등등
        *   4. 구조 : '큐' 라는 자료구조를 배치하여 순서대로 스레드풀에서 대기중인 스레드에게 작업 배정
        *       - 큐 란? 지려의 데이터를 요청 순서대러 처리방법, 먼저 들어온 요청을 먼저 처리한다
        *       --------------------------------------
        ...
        *       콜 요청 --------->       직원2(강호동/스레드)
        *                               직원3(신동엽/스레드)
        *   만약에 2개의 콜의 동시에 들어오면 2명이 각각 배정되고 직원 1명 대기
        *   만약에 3개의 콜의 동시에 들어오면 3명이 각각 배정되고 콜요청 2개 대기
        *
