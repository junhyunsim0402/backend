...
-- error select bpname,bprice from buy group by bpanme;

#[2]집계함수
select sum(bamount)from buy; -- buy테이블내 amount속성값의 합계
select avg(bamount)from buy; -- buy테이블내 amount속성값의 평균
select min(bamount)from buy; -- buy테이블내 amount속성값의 최소값
select max(bamount)from buy; -- buy테이블내 amount속성값의 최댓값
select count(bamount)from buy; -- buy테이블내 amount속성값의 전체 레코드 수(null제외)
select count(*) from buy; -- buy테이블내 amount속성값의 전체 레코드 수(null제외)

#[3]그룹절과 집계함수
# 1) 회원아이디별 구매수량 합계
select * from buy;					-- buy 테이블 전체 조회
select mid from buy group by mid;	-- buy 테이블 mid기준으로 그룹조회
select mid,sum(bamount) from buy group by mid;	-- -- buy 테이블 mid기준으로 그룹화하여 bamount총합계
# 2) 회원아이디별로 총 구매금액(구매수량*구매가격)
select mid,sum(bamount*bprice)from buy group by mid;
# 3) 총 판매횟수, 회원아이디별로 판매횟수
select count(*) from buy;
select mid,count(*) from buy group by mid;

# 4) having : 그룹절의 조건절, where : 그룹 전 조건 VS having:그룹 후 조건
# 1) 그룹 전 조건 where		,구매수량(그룹전에 존재하는 속성)이 3이상인
...
# 주의할점: 다중 정렬은 order by 1차정렬기준,2차정렬기준
	# -> 1차정렬기준으로 동일한 값이 있을때 2차정렬
select * from member order by mdebut asc;
select * from member order by mdebut desc;
	# -> 1차정렬에서 maddr(주소) 정렬 한 후 동일한 maddr(주소끼리) mdebut(날짜) 정렬
select * from member order by maddr desc, mdebut asc;

# [6] limit : 조회 결과 제한,(검색/페이징) / limit 에 시작 번호, 개수
select * from member limit 2; -- 조회결과 위에서부터 2개만 조회
select * from member limit 0,2; -- 조회결과 0(첫번째 레코드)부터 2개 조회
select * from member limit 0,5;	-- 1부터 5개 조회
select * from member limit 5,5;	-- 5부터 5개

# select 작성 규칙/순서
# select 속성명 form 테이블명 where 일반조건 group by 그룹기준 having 그룹조건 order by 정렬기준 limit 개수
# select 처리순서
